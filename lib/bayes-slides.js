...
(function () {
      ...
      '<div class="win reveal">' + winbar(bi("Analytique", "Analytics"), bi("30 derniers jours", "last 30 days")) +
        '<div class="win-body"><div class="metric-grid">' +
          ...
          '<div class="metric"><div class="m-lab">' + bi("Sujets · couverture documentaire", "Topics · document coverage") + "</div>" +
            ...
            "</div></div>" +
        "</div></div></div>" +
      '<div class="incident reveal" style="margin-top:16px;">' +
        '<div class="inc-ico">!</div><div>' +
          '<div class="inc-src">' + bi("INCIDENT IA · OECD.AI · 2024", "AI INCIDENT · OECD.AI · 2024") + "</div>" +
          '<div class="inc-head">' + bi(
            "Un service public de l'emploi a déployé un chatbot reproduisant des biais de genre.",
            "A public employment service shipped a chatbot that reproduced gender bias.") + "</div>" +
          '<div class="inc-sub">' + bi(
            "Le monter soi-même, c'est mille subtilités à maîtriser : le dilemme entre souveraineté et réinventer la roue. On surveille les analytics et la qualité du RAG précisément pour éviter cela. ",
            "Building it yourself means a thousand subtleties to get right: the dilemma between sovereignty and reinventing the wheel. We monitor analytics and RAG quality precisely to prevent this. ") +
            '<a href="https://oecd.ai/en/incidents/2024-01-04-47fe" target="_blank" rel="noopener" style="color:var(--accent-blue);font-weight:600;text-decoration:none;white-space:nowrap;">oecd.ai ↗</a>' + "</div>" +
        "</div></div>" +
      "</div></section>";
  };
  ...
  S["platform-compliance"] = function () {
    ...
    return '<section class="slide" id="platform-compliance"><div class="slide-inner">' +
      ...
      '<div class="reveal" style="display:flex;flex-direction:column;max-width:880px;margin-top:14px;padding:14px 14px 6px;border-radius:18px;background:linear-gradient(to top, rgba(255,132,0,.08), rgba(255,132,0,0) 58%);">' +
        ...
        '<div style="background:var(--ink);color:#fff;border-radius:14px;padding:15px 18px;box-shadow:0 0 0 2px rgba(255,132,0,.16), var(--shadow);">' +
          ...
          '<div style="display:flex;align-items:center;gap:9px 12px;flex-wrap:wrap;margin-top:11px;padding-top:11px;border-top:1px solid rgba(255,255,255,.14);">' +
            '<span style="font-family:\'JetBrains Mono\',monospace;font-size:9px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;color:rgba(255,255,255,.55);">' + bi("Sert les derniers modèles · indépendant du fournisseur", "Serves the latest models · provider-independent") + "</span>" +
            chip("Frontier · cloud", "Frontier · cloud", 1) + chip("Open-weight : Gemma · Mistral · Qwen", "Open-weight: Gemma · Mistral · Qwen", 1) +
          "</div>" +
        "</div>" +
