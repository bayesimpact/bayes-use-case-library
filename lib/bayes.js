...
(function () {
  ...
  function chrome() {
    ...
    if (dotsWrap) dotsWrap.classList.toggle("on-dark", slides[cur].hasAttribute("data-dark"));
  }
  ...
  /* ---------------- earth hero (starfield + orbit + fade-cycle) ---------------- */
  (function () {
    var box = document.getElementById("believeBox"); if (!box) return;
    for (var i = 0; i < 64; i++) { var st = document.createElement("span"); st.className = "star"; var z = (Math.random() * 1.8 + 0.7).toFixed(1); st.style.cssText = "width:" + z + "px;height:" + z + "px;left:" + (Math.random() * 100).toFixed(2) + "%;top:" + (Math.random() * 100).toFixed(2) + "%;--tw:" + (Math.random() * 3 + 2).toFixed(1) + "s;animation-delay:" + (Math.random() * 3).toFixed(1) + "s"; box.appendChild(st); }
    var orbit = box.querySelector("#planetOrbit");
    if (orbit) [{ r: 36, n: 16, d: 90 }, { r: 43, n: 22, d: 130, rev: 1 }, { r: 49, n: 28, d: 185 }].forEach(function (R) {
      var ring = document.createElement("div"); ring.className = "oring" + (R.rev ? " rev" : ""); ring.style.animationDuration = R.d + "s";
      for (var k = 0; k < R.n; k++) { var a = ((360 / R.n) * k + Math.random() * 12) * Math.PI / 180, rr = R.r + (Math.random() * 3 - 1.5); var dt = document.createElement("span"); dt.className = "odot"; var s = (Math.random() * 1.5 + 0.7).toFixed(1); dt.style.cssText = "left:" + (50 + rr * Math.cos(a)).toFixed(2) + "%;top:" + (50 + rr * Math.sin(a)).toFixed(2) + "%;width:" + s + "px;height:" + s + "px;opacity:" + (Math.random() * .5 + .35).toFixed(2); ring.appendChild(dt); }
  ...
  /* ---------------- language toggle + PDF button ---------------- */
  if (LANGS.length > 1) {
    var lt = document.createElement("button"); lt.className = "lang-toggle"; lt.type = "button";
    lt.textContent = LANGS[(LANGS.indexOf(lang) + 1) % LANGS.length].toUpperCase();
    lt.addEventListener("click", function () { setLang(LANGS[(LANGS.indexOf(lang) + 1) % LANGS.length]); });
    document.body.appendChild(lt);
    function setLang(l) { lang = l; localStorage.setItem("bayesLang", l); applyLang(); lt.textContent = LANGS[(LANGS.indexOf(lang) + 1) % LANGS.length].toUpperCase(); Believe.lang(); for (var k in DEMOS) { if (/^\d+$/.test(k) && DEMOS[k].setLang) DEMOS[k].setLang(); } enter(cur); }
  }
  var pdf = document.createElement("button"); pdf.className = "pdf-btn"; pdf.type = "button"; pdf.textContent = "PDF";
  pdf.addEventListener("click", function () { preparePrint(); setTimeout(function () { window.print(); }, 60); });
  document.body.appendChild(pdf);
  function preparePrint() { slides.forEach(function (s) { s.classList.add("show"); }); for (var k in DEMOS) { if (/^\d+$/.test(k) && DEMOS[k].renderFinal) DEMOS[k].renderFinal(); } }
  // also instantiate demos so renderFinal exists, then prepare on native print
